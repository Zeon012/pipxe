...
```
* The image includes cloud-init first-boot seed files from the attached Pi boot drive root (`user-data`, `network-config`, and `meta-data`). The network seed now configures the Pi 3's built-in `eth0` as `192.168.72.1/24` so PXE serves over the onboard Ethernet port on first boot.
